...
QsvUnpacker::QsvUnpacker(const QString& file_path) {
  ...
  if (memcmp(qheader.signature, QSV_SIGNATURE, sizeof(QSV_SIGNATURE) - 1) !=
      0) {
    errcode = -3;
  ...
  if (infile->read((char*)qindices, nb_indices * sizeof(QsvIndex)) !=
      nb_indices * sizeof(QsvIndex)) {
    errcode = -2;
    msg = "Incomplete file";
    return;
  }
...
int QsvUnpacker::get_errcode() const { return errcode; }

const QString& QsvUnpacker::get_msg() const { return msg; }

int QsvUnpacker::get_nb_indices() const { return nb_indices; }
