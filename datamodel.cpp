...
DataModel::DataModel(QTableView* tbvInputList, QLineEdit* txtOutputDir) {
  ...
  inputFileModel->setHorizontalHeaderLabels({"Filename", "Status"});
...
void DataModel::appendInputFile(const QString& filePath) {
  ...
  inputFileModel->appendRow({new QStandardItem(file.getFileName()),
                             new QStandardItem(file.getStatusMsg())});
}
...
const QDir& DataModel::getOutputDir() const { return outputDir; }
...
int DataModel::getGlobalStatus() const { return globalStatus; }

void DataModel::setGlobalStatus(int value) { globalStatus = value; }
...
QVector<InputFileModel>* DataModel::getInputFiles() { return &files; }

const QString& DataModel::getTargetFormat() const { return targetFormat; }

void DataModel::setTargetFormat(const QString& value) { targetFormat = value; }
