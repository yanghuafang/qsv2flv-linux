...
void ConverterThread::run() { convertAllFiles(); }
...
void ConverterThread::convertSingleFile() {
  ...
  QString outputName =
      curFile->getFileBaseName() + dataModel->getTargetFormat();
...
AVFormatContext* ConverterThread::createInputContext(QsvUnpacker* unpacker) {
  ...
  inCtx->pb = avio_alloc_context(buffer, BUFFER_SIZE, 0, unpacker,
                                 readFromSegment, nullptr, nullptr);
  ...
  if (avformat_find_stream_info(inCtx, 0) < 0) {
    curFile->setStatusCode(InputFileModel::STATUS_FAILED);
    curFile->setStatusMsg("Unknown stream format");
    emit fileStatusChanged(curRowIndex);
    avformat_close_input(&inCtx);
    return nullptr;
  }
...
AVFormatContext* ConverterThread::createOutputContext(const char* outputPath,
  ...
  for (unsigned int i = 0; i < inCtx->nb_streams; ++i) {
    ...
    if (!outCodec) {
      curFile->setStatusCode(InputFileModel::STATUS_FAILED);
      curFile->setStatusMsg("Unsupported stream format");
      emit fileStatusChanged(curRowIndex);
      avformat_free_context(outCtx);
      return nullptr;
    }
    ...
    if (avcodec_parameters_from_context(out_stream->codecpar, out_codec_ctx) <
        0) {
      curFile->setStatusCode(InputFileModel::STATUS_FAILED);
  ...
  if (avio_open(&outCtx->pb, outputPath, AVIO_FLAG_WRITE) < 0) {
    curFile->setStatusCode(InputFileModel::STATUS_FAILED);
    curFile->setStatusMsg("Unable to create output file");
    emit fileStatusChanged(curRowIndex);
    avformat_free_context(outCtx);
    return nullptr;
  }
...
int ConverterThread::copyStreams(AVFormatContext* outCtx,
                                 AVFormatContext* inCtx,
                                 QsvUnpacker* unpacker) {
  AVPacket pkt;
